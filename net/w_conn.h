...
namespace wlib {
	...
	static bool operator<(const peer& p1, const peer& p2) {
	...
	struct w_respond_writer {
		...
		int write(w_respond* res) {
			...
			if (n != HEADER_LENGTH) {
				w_dbg(CRIT, "write header error in w_respond_writer::write()");
            return -1;
			}
			n = rw_.write(res->body_, res->h_.body_len_);
			if (n != res->h_.body_len_) {
				w_dbg(CRIT, "write data error in w_respond_writer::write()");
            return -1;
			}
			return 0;
		}
   ...
   public:
		...
		virtual int serve_request(w_respond_writer w, w_request* req) {
         ...
         handler_key key = req->h_.req_type_;
			...
			if (it == handler_map_.end()) {
				w_dbg(CRIT, "no match the request handler function");
            return -1;
			}
		...
		virtual int serve_respond(w_respond* r) {
			...
			IT it = handler_map_.find(key);
			if (it == handler_map_.end()) {
				w_dbg(CRIT, "no match the respond handler function");
            return -1;
			}
	...
	template<class T>
      ...
      peer peer_;
		...
		virtual int read_handler(w_socket s) {
			...
			if (n != HEADER_LENGTH) {
				w_dbg(CRIT, "read header error in w_conn::read_handler()");
            return -1;
			}
			...
			if (n != bodylen) {
				w_dbg(CRIT, "read body error in w_conn::read_handler()");
            return -1;
			}

			if (header.isreq_ == 1) { // is a respond
				w_respond res;
				res.h_ = header;
				res.body_ = body;
		...
		int write_request(w_request* r) {
			...
			if (n != HEADER_LENGTH) {
				w_dbg(CRIT, "write header error in w_conn::write_request()");
            return -1;
			}
			n = rw_.write(r->body_, bodylen);
			if (n != bodylen) {
				w_dbg(CRIT, "write body error in w_conn::write_request()");
            return -1;
			}
			return 0;
		}
